  ...
  return (
    <Container>
      ...
      <Content>
        ...
        <HotelsList>
          <div>
            <HotelsTitle>Hôtels</HotelsTitle>
            <HotelCount>{hotels.length}</HotelCount>
          </div>
          <CreateHotelButton onClick={() => router.push("/hotelform")}>
        ...
        <ContentWrapper>
          <Grid>
            {hotels.map((hotel, index) => (
              <Link href={`hotel/${hotel.photo}`} key={index}>
                <Card>
                  <Image
                    src={`/uploads/${encodeURIComponent(hotel.photo)}`}
                    alt={hotel.title}
                    width={300}
                    height={300}
